...
/// # use cranelift_frontend::{FunctionBuilder, FunctionBuilderContext, Position, Switch};
/// #
/// # let mut sig = Signature::new(CallConv::SystemV);
/// # let mut fn_builder_ctx = FunctionBuilderContext::new();
/// # let mut func = Function::with_name_signature(ExternalName::user(0, 0), sig);
/// # let mut position = Position::default();
/// # let mut builder = FunctionBuilder::new(&mut func, &mut fn_builder_ctx, &mut position);
/// #
...
mod tests {
    use super::*;
    use crate::frontend::{FunctionBuilderContext, Position};
    ...
    macro_rules! setup {
        ($default:expr, [$($index:expr,)*]) => {{
            ...
            let mut position = Position::default();
            {
                let mut bx = FunctionBuilder::new(&mut func, &mut func_ctx, &mut position);
                let ebb = bx.create_ebb();
                bx.switch_to_block(ebb);
